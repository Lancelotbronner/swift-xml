...
open class XMLEncoder {
	...
	public struct OutputFormatting : Sendable, OptionSet {
...
class _XMLEncoder: Encoder {
	...
	init(options: XMLEncoder._Options, codingPath: [CodingKey] = []) {
...
fileprivate struct _XMLKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
	...
	private func _converted(_ key: CodingKey) -> CodingKey {
		...
		case .useDefaultKeys:
			return key
		case .convertToSnakeCase:
			return _CodingKey(codingKey: key)
				.mapString(do: XMLEncoder.KeyEncodingStrategy._convertToSnakeCase)
	...
	public mutating func superEncoder() -> Encoder {
		return _XMLReferencingEncoder(referencing: self.encoder, key: _CodingKey.super, convertedKey: _converted(_CodingKey.super), wrapping: self.container)
	}
...
fileprivate struct _XMLUnkeyedEncodingContainer : UnkeyedEncodingContainer {
	...
	public mutating func encode(_ value: Float)  throws {
		// Since the float may be invalid and throw, the coding path needs to contain this key.
		self.encoder.codingPath.append(_CodingKey(intValue: self.count))
		defer { self.encoder.codingPath.removeLast() }
	...
	public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type) -> KeyedEncodingContainer<NestedKey> {
		self.codingPath.append(_CodingKey(intValue: self.count))
		defer { self.codingPath.removeLast() }
