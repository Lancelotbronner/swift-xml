...
class _XMLDecoder : Decoder {
	...
	init(referencing container: Any, at codingPath: [CodingKey] = [], from decoder: XMLDecoder) {
...
extension _XMLDecoder {
	...
	func unbox(_ value: Any, as type: Float.Type) throws -> Float? {
		...
		if let value = Double(string) {
			...
			return Float(double)
		} else if case let .convertFromString(posInfString, negInfString, nanString) = self.decoder.nonConformingFloatDecodingStrategy {
