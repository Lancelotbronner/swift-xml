...
extension _XMLKeyedDecodingContainer : KeyedDecodingContainerProtocol {
	...
	@inlinable public func decode<T : Decodable>(_ type: T.Type, forKey key: Key) throws -> T {
		try storage.with(key) {
			try storage.decodeCompound()
		}
	}
	
	@inlinable public func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: Key) throws -> KeyedDecodingContainer<NestedKey> {
		try storage.with(key) {
			storage.keyedContainer()
		}
	}
	
	@inlinable public func nestedUnkeyedContainer(forKey key: Key) throws -> UnkeyedDecodingContainer {
		try storage.with(key) {
